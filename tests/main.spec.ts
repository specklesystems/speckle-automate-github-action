...
// A representative 422 error body, as returned by the automate server when a
// request fails its own (server-side) schema validation.
const error422 = {
  type: 'H3Error',
...
describe('Register new version', () => {
  ...
  // Stub each provided environment variable. A value of `null` unsets the
  // variable entirely, letting a test simulate a genuinely absent input.
  const applyEnv = (env: Record<string, string | null>): void => {
    for (const [key, value] of Object.entries(env)) {
      vi.stubEnv(key, value === null ? undefined : value)
    }
  }

  // Replace the happy-path handler with one that records the request body, so a
  // test can assert on what the action actually sent. Returns a holder whose
  // `.body` is populated once the request has been made.
  const captureRequestBody = (): { body?: Record<string, unknown> } => {
  ...
  // The default handler: a valid request to the happy-path function id succeeds.
  // Tests that need other behaviours (errors, retries, body capture) install
  // their own handler with `server.use(...)`.
  ...
  describe('request and outputs', () => {
    ...
    it('omits the recommended CPU and memory from the request body when not provided', async () => {
      ...
      expect(captured.body).toBeDefined()
      expect('recommendedCPUm' in (captured.body as object)).toBe(false)
      expect('recommendedMemoryMi' in (captured.body as object)).toBe(false)
    })
    ...
    it('accepts an absolute input schema file path', async () => {
      ...
      await expect(run()).resolves.not.toThrow()
      expect(captured.body).toMatchObject({ inputSchema: { absolute: true } })
    })
  })
  ...
  describe('input validation', () => {
    it('errors if the token is empty', async () => {
      ...
      // core.setFailed is what actually marks the action step as failed in CI
      expect(setFailed).toHaveBeenCalledWith(
        expect.stringContaining('Input required and not supplied: speckle_token')
      )
    })

    it('errors if the git commit sha is empty', async () => {
      const setFailed = vi.spyOn(core, 'setFailed')
      writeFileSync(join(tmpDir, 'schema.json'), '{}')
      applyEnv({ ...baseEnv(tmpDir), GITHUB_SHA: '' })
      await expect(run()).rejects.toThrow('gitCommitSha')
      expect(setFailed).toHaveBeenCalledWith(expect.stringContaining('gitCommitSha'))
    })
    ...
    it('rejects a release tag that violates the OCI tag format', async () => {
      ...
      applyEnv({
        ...baseEnv(tmpDir),
        INPUT_SPECKLE_FUNCTION_RELEASE_TAG: '-invalid-tag'
      })
    ...
    it('accepts a release tag of exactly 128 characters', async () => {
      ...
      await expect(run()).resolves.not.toThrow()
      expect(captured.body).toMatchObject({ versionTag: tag })
    })
  ...
  describe('HTTP and retry behaviour', () => {
    it('handles network errors', async () => {
      const setFailed = vi.spyOn(core, 'setFailed')
      server.use(http.post(versionsUrl('fake_function_id'), () => HttpResponse.error()))
      writeFileSync(join(tmpDir, 'schema.json'), '{}')
      applyEnv(baseEnv(tmpDir))
      await expect(run()).rejects.toThrow(
      ...
      expect(setFailed).toHaveBeenCalledWith(
        expect.stringContaining('Failed to register new function version')
      )
    })
    ...
    it('retries on 500 responses', async () => {
      let attempts = 0
      server.use(
        http.post(versionsUrl('fake_function_id'), () => {
          attempts++
      ...
      expect(attempts).toBeGreaterThan(1) // we expect the action to retry the request
    })
    ...
    it('retries exactly maxAttempts (5) times on repeated 500 responses', async () => {
      ...
      // `maxAttempts: 5` in the retry configuration means the initial attempt
      // plus four retries: five requests in total.
      expect(attempts).toBe(5)
    })

    it('does not retry on a 422 response', async () => {
      let attempts = 0
      server.use(
        http.post(versionsUrl('fake_function_id'), () => {
          attempts++
          return HttpResponse.json(error422, { status: 422 })
        })
      )
      writeFileSync(join(tmpDir, 'schema.json'), '{}')
      applyEnv(baseEnv(tmpDir))
      ...
      expect(attempts).toBe(1) // a 4xx must not be retried
    })
    ...
    it('sends a memory value the client accepts but the server rejects (client/server bound mismatch)', async () => {
      // The client schema permits recommendedMemoryMi up to 60000, but the
      // server schema (mirrored by FunctionVersionRequestSchema) only permits up
      // to 8000. A value in that gap passes client validation and is sent, but
      // the server rejects it with a 422. This documents the divergent bounds.
      let attempts = 0
      let serverRejected = false
      server.use(
        http.post(versionsUrl('fake_function_id'), async ({ request }) => {
          attempts++
          const parseResult = FunctionVersionRequestSchema.safeParse(
            await request.json()
          )
      ...
      // the value was actually sent and rejected by the server (not caught by
      // client-side validation), and the resulting 422 was not retried
      expect(serverRejected).toBe(true)
      expect(attempts).toBe(1)
    })
  })
