...
var ValidationError_1 = __nccwpck_require__(3311);
...
function throwErrorOnClientErrorStatusCode(apiRequest) {
    return async () => {
        ...
        if (response.status >= 400 && response.status < 500)
            throw new NonRetryableError(`Status code indicates a client error. Not retrying. (${response.status}; ${response.body})`);
        if (response.status >= 500)
            throw new RetryableError(`Status code indicates a server error. Retrying. (${response.status}; ${response.body})`);
        return response;
    };
}
